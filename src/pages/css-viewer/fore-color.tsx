...
import { FormNumeric, FormContext, useFormContext, FormContextProps } from '../../components'
...
type IProps = { bgColor: string }
export const ForeColor = observer(({ bgColor }: IProps) => {
    ...
    return (
        <PaletteRoot style={{ position: 'relative', userSelect: dragging ? 'none' : 'unset' }}>
            <ColorPalette>
                <HueRound>
                    <Hue
                        style={{
                    ...
                    {[30, 60, 120, 180, 240, 280, 300, 330].map((it) => (
                        <Degree
                            ...
                            style={{
                                ...
                                userSelect: 'none',
                                borderColor: [30, 60, 120, 180].includes(it) ? 'black' : 'white',
                            }}
                        >
                            <span
                                className="text"
            ...
            <ColorOptions>
                ...
                {[0, 1, 2, 3, 4, 5].reverse().map((it) => {
                    ...
                    return (
                        <ColorLabel bgColor={color} foreColor={bgColor} key={`Lightness-${it}`} />
                    )
                })}
            </ColorOptions>
            <ColorOptions>
                ...
                {[1, 2, 3, 4, 5, 6].map((it) => {
                    ...
                    return <ColorLabel bgColor={color} foreColor={bgColor} key={`Darkness-${it}`} />
                })}
            </ColorOptions>
        </PaletteRoot>
    )
